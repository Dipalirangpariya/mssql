...
const ApiReaponse_1 = require("./ApiReaponse");
var ErrorType;
(function (ErrorType) {
    ...
    static handle(err, res) {
        switch (err.type) {
            ...
            case ErrorType.UNAUTHORIZED:
                return new ApiReaponse_1.AuthFailureResponse(err.message).send(res);
            case ErrorType.ACCESS_TOKEN:
                return new ApiReaponse_1.AccessTokenErrorResponse(err.message).send(res);
            case ErrorType.INTERNAL:
                return new ApiReaponse_1.InternalErrorResponse(err.message).send(res);
            case ErrorType.NOT_FOUND:
            case ErrorType.NO_ENTRY:
            case ErrorType.NO_DATA:
                return new ApiReaponse_1.NotFoundResponse(err.message).send(res);
            case ErrorType.BAD_REQUEST:
                return new ApiReaponse_1.BadRequestResponse(err.message).send(res);
            case ErrorType.FORBIDDEN:
                return new ApiReaponse_1.ForbiddenResponse(err.message).send(res);
            default: {
                let message = err.message;
                // Do not send failure message in production as it may send sensitive data
                if (config_1.environment === 'production')
                    message = 'Something wrong happened.';
                return new ApiReaponse_1.InternalErrorResponse(message).send(res);
            }
        }
    }
}
